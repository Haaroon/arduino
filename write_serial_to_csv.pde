...
int numReadings = 75; //keeps track of how many readings you'd like to take before writing the file. 
...
void setup()
{
  ...
  table.addColumn("thermister_tempc");
  table.addColumn("dht_temp");
  table.addColumn("dht_humidity");
  //fileName = "/tmp/"+str(year()) + str(month()) + str(day())+".csv"; //this filename is of the form year+month+day+readingCounter
...
void serialEvent(Serial myPort){
  ...
  if (val!= null) { //We have a reading! Record it.
    ...
    newRow.setFloat("thermister_tempc", sensorVals[0]);
    newRow.setFloat("dht_temp", sensorVals[1]);
    newRow.setFloat("dht_humidity", sensorVals[2]);
    ...
    {
      //print("Saving file\n");
      fileName = "/tmp/"+str(year())+"-"+ str(month())+"-"+ str(day())+"-"+str(hour())+"-"+str(minute())+str(second())+".csv"; //this filename is of the form year+month+day+readingCounter
      saveTable(table, fileName); //Woo! save it to your computer. It is ready for all your spreadsheet dreams. 
      table.clearRows();
    }
   }
   //print("Serial Event Null\n");
}
